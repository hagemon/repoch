...
def fetch_and_parse_to_markdown(url):
    ...
    # 找到指定的容器元素
    container = doc('.prose-custom.prose-custom-md.prose-custom-gray.\\!max-w-none.text-neutral-300.\\[overflow-wrap\\:anywhere\\]')
    
    if not container:
        return "未找到指定的元素"
    
    # 提取标题和段落元素
    elements = container.find('h1, h2, h3, h4, h5, h6, p, ol, ul, li, table, thead, tbody, tr, th, td')
    ...
    for el in elements.items():
        ...
        # 列表结束
        if tag in ['ol', 'ul'] and el.next() and el.next()[0].tag not in ['ol', 'ul', 'li']:
            if list_stack:
                list_stack.pop()
                number_stack.pop()
            markdown += '\n'
...
def get_wiki(language, date):
    ...
    with open(os.path.join(filtered_path, f"{language}.json"), "r", encoding="utf-8") as f:
        repos = json.load(f)
    for repo in repos:
